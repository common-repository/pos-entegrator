...
class Insights extends \GurmeHub\Api {
	...
	public function upgrader_process_complete( $upgrader, $hook_extra ) {
		...
		if ( $upgrader instanceof Plugin_Upgrader && false === $upgrader->bulk && array_key_exists( 'plugin', $hook_extra ) && $hook_extra['plugin'] === $this->plugin->get_basename() ) {
			$this->send_tracking_data();
		}

		if ( $upgrader instanceof Plugin_Upgrader && true === $upgrader->bulk && array_key_exists( 'plugins', $hook_extra ) && in_array( $this->plugin->get_basename(), $hook_extra['plugins'], true ) ) {
			$this->send_tracking_data();
		}
	}
